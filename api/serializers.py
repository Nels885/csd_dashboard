...
class ProgSerializer(serializers.ModelSerializer):
    ...
    swap_prod = serializers.CharField(source='swap_prod.name', read_only=True, default='')
    ...
    prod_name = serializers.ReadOnlyField(source='corvet.prods.btel.get_name_display')
    ...
    class Meta:
        model = Xelon
        fields = (
            'xelon', 'vin', 'xelon_model', 'xelon_vehicle', 'hw_ref', 'uin_sn', 'swap_sn', 'swap_prod', 'cal',
            'cal_new', 'product', 'prod_name', 'level', 'type'
        )
        read_only_fields = ['vin', 'swap_sn']
